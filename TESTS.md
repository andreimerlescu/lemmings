...
arbitrary bytes from live HTTP servers. These are the functions that, in
production, receive content from servers that lemmings does not control. A
web server under load may return truncated responses, malformed HTML, corrupted
XML, or waiting room pages with unexpected structure. The fuzz tests prove that
lemmings handles all of these gracefully.
